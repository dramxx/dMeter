...
pub fn render_memory(f: &mut Frame, area: Rect, data: &MemoryData, show_swap: bool) {
    ...
    // Calculate bar width dynamically based on text content
    let text_content = format!("RAM  [] {} / {}", mem_used, mem_total);
    let text_width = text_content.len() + 2;
    ...
    if show_swap && data.swap_total > 0 {
        ...
        // Calculate bar width dynamically based on text content
        let swap_text_content = format!("SWAP [] {} / {}", swap_used, swap_total);
        let swap_text_width = swap_text_content.len() + 2;
        let swap_bar_width = (inner.width as usize).saturating_sub(swap_text_width).max(10);
        
        let swap_bar = render_bar(swap_percent, swap_bar_width);
